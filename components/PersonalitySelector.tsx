...
const personalities = [
  { id: 'default', emoji: '🔮', name: 'Sarcastic' },
  { id: 'cat', emoji: '😼', name: 'Catstrology' },
  { id: 'dog', emoji: '🐶', name: 'Zen Dog' },
] as const;


const PersonalitySelector: React.FC<PersonalitySelectorProps> = ({ selectedPersonality, onPersonalitySelect }) => {
    ...
    return (
        <div className="flex justify-center items-center gap-4 sm:gap-6 w-full max-w-md mx-auto">
            {personalities.map((p) => {
                ...
                return (
                    <button
                        ...
                        aria-label={`Select ${p.name} guide`}
                    >
